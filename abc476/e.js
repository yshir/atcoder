...
const min_st = new LazySegmentTree(
  P, // array
  Math.min, // op
  Infinity, // e
  (f, x) => (f === null ? x : f), // mapping
  (f, g) => (f === null ? g : f), // composition
  null, // id
);

const max_st = new LazySegmentTree(
  P, // array
  Math.max, // op
  -Infinity, // e
  (f, x) => (f === null ? x : f), // mapping
...
for (let i = 0; i < M; i++) {
  let [l, r] = input[i + 2].split(' ').map(Number);
  l--;
  r--;

  const min = min_st.prod(l, r + 1);
  const max = max_st.prod(l, r + 1);

  const min_i = map.get(min);
  const max_i = map.get(max);

  min_st.apply(max_i, min);
  min_st.apply(min_i, max);
  max_st.apply(max_i, min);
  max_st.apply(min_i, max);
