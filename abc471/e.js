...
class ModComb {
  ...
  nCr_large(n, r) {
    ...
    return this.mulMod(num, this.#invFact[r]);
  }
...
const mc = new ModComb(MOD, 1e6);

const A_acc = [...A];
for (let i = N - 2; i >= 0; i--) {
  A_acc[i] = mc.addMod(A_acc[i], A_acc[i + 1]);
}

let ans = 0;
for (let i = 0; i < N; i++) {
  if (N - 1 >= 0 && K - 1 >= 0) {
    let cur = mc.nCr(N - 1, K - 1);
    cur = mc.mulMod(cur, A[i]);
    cur = mc.mulMod(cur, A[i]);
    ans = mc.addMod(ans, cur);
  }

  if (N - 2 >= 0 && K - 2 >= 0 && i < N - 1) {
    let cur = mc.mulMod(2, A[i]);
    cur = mc.mulMod(cur, A_acc[i + 1]);
    cur = mc.mulMod(cur, mc.nCr(N - 2, K - 2));
    ans = mc.addMod(ans, cur);
  }
}
console.log(ans);
