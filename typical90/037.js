...
const INF = 1e9 * 500 + 10;

const segtree = (arr) => {
  const op = (a, b) => Math.max(a, b);
  const e = -INF;
  const mapping = (f, x) => f + x;
  const composition = (f, g) => f + g;
  const id = 0;
  return new LazySegmentTree(arr, op, e, mapping, composition, id);
};

let dp = Array(W + 1).fill(-INF);
dp[0] = 0;
let st = segtree(dp);
...
for (let i = 0; i < N; i++) {
  ...
  for (let j = 0; j <= W; j++) {
    const pr = j - l;
    const pl = Math.max(j - r, 0);
    if (pr < 0) continue;
    const nv = st.prod(pl, pr + 1) + v;
    next_dp[j] = Math.max(next_dp[j], nv);
  }
  dp = next_dp;
  st = segtree(next_dp);
}

console.log(dp[W] < 0 ? -1 : dp[W]);
