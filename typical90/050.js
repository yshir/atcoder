...
class ModComb {
  ...
  f(a, b) {
    return this.mulMod(this.mulMod(this.#fact[a + b], this.#invFact[a]), this.#invFact[b]);
  }
...
const lcmax = Math.floor(N / L);

let ans = 0;
const mc = new ModComb(M, 100000);
for (let lc = 0; lc <= lcmax; lc++) {
  const oc = N - lc * L;
  ans += mc.f(lc, oc);
  ans %= M;
}
