...
for (let i = 0; i < H; i++) {
  const t = new Treap();
  for (let j = 0; j < W; j++) t.insert(j);
  A_H[i] = t;
}
for (let i = 0; i < W; i++) {
  const t = new Treap();
  for (let j = 0; j < H; j++) t.insert(j);
  A_W[i] = t;
}
...
for (let i = 0; i < Q; i++) {
  ...
  if (A_H[r].delete(c)) {
    ...
    // UDLR
    {
      const idx = A_H[r].rank(c);

      const c2 = A_H[r].kth(idx);
      const c3 = A_H[r].kth(idx - 1);
    ...
    {
      const idx = A_W[c].rank(r);

      const r2 = A_W[c].kth(idx);
      const r3 = A_W[c].kth(idx - 1);
