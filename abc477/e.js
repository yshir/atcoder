...
while (!pq.empty()) {
  const [u, pd] = pq.dequeue();
  if (D[u] !== undefined) continue;
  D[u] = pd;
...
for (let i = 0; i < Q; i++) {
  let [s, t] = input[i + 3].split(' ').map(Number);
  s--;
  t--;
  ...
  if (t === N) {
    ...
    const a = ACC[t - 1] - (ACC[s - 1] || 0n);
    const b = SUM - a;
    const c = D[s] + D[t];
    console.log(min_f(a, min_f(b, c)).toString());
  }
}
