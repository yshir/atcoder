 ...
 * @returns {number} (a / b) mod MOD, in [0, MOD).
 */
const divMod = (a, b) => mulMod(a, invMod(b, MOD), MOD);

const ps = new PrimeSieve(1e7);
...
while (T--) {
  ...
  const B = [];
  for (let i = 0; i < N; i++) {
    B[i] = ps.factorize2(A[i]);
  }

  const map = new Map();
  for (let i = 0; i < N; i++) {
    for (let j = 0; j < B[i].length; j += 2) {
      const p = B[i][j];
      const r = B[i][j + 1];
      if (map.get(p) === undefined) {
        map.set(p, [r, 0]);
      } else {
        const [v1, v2] = map.get(p);
        if (r > v1) {
          map.set(p, [r, v1]);
        } else if (r > v2) {
          map.set(p, [v1, r]);
  ...
  for (const [p, v] of map) {
    base = mulMod(base, powMod(p, v[0]));
  }

  const corr = new Map();
  for (const [p, v] of map) {
    corr.set(p, invMod(powMod(p, v[0] - v[1])));
  }
...
console.log(ans2.map((x) => x.join(' ')).join('\n'));
