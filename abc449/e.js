...
const queries = [];
for (let i = 0; i < Q; i++) {
  let [x] = input[i + 3].split(' ').map(BigInt);
  x -= 1n;
  if (x < BigInt(N)) {
    ans[i] = A[Number(x)];
  } else {
...
queries.sort((a, b) => (b[0] - a[0] < 0n ? -1 : 1));
...
for (let i = 0; i <= N; i++) {
  for (const n of map[i]) t.insert(n);
  const r = l + BigInt(t.size);
  while (queries.length > 0) {
    if (queries[queries.length - 1][0] >= r) break;
    const [x, qi] = queries.pop();
    ans[qi] = t.kth(Number(x - l));
  }
  l = r;
}

for (let [x, qi] of queries) {
  ans[qi] = t.kth(Number((x - l) % BigInt(M)));
}
